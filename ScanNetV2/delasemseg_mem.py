...
class Stage(nn.Module):
    ...
    def forward(self, x, xyz, prev_knn, indices, pts_list, y, epoch_ratio):
        """
        x: N x C
        """
        # downsampling
        ...
        if self.have_memory:
            ...
            coarse_seg_loss = 0
            if self.training:
                coarse_seg_loss = F.cross_entropy(coarse_pred, y, label_smoothing=0.2, ignore_index=20)
                self.memory.update(x, y, coarse_pred, epoch_ratio)
            x, _ = self.memory(x, coarse_pred)

        # get subsequent feature maps
        if not self.last:
            sub_x, sub_c, sub_coarse_loss = self.sub_stage(x, xyz, knn, indices, pts_list, y, epoch_ratio)
        else:
            sub_x = sub_c = sub_coarse_loss = None
...
class DelaSemSeg_Mem(nn.Module):
    r"""
    ...
    def forward(self, xyz, x, indices, pts_list=None, y=None, epoch_ratio=None):
        indices = indices[:]
        x, closs, coarse_seg_loss = self.stage(x, xyz, None, indices, pts_list, y, epoch_ratio)
        if self.training:
            return self.head(x), closs, coarse_seg_loss
        return self.head(x)
