...
class Stage(nn.Module):
    def __init__(self, args, depth=0):
        ...
        if self.depth in args.memory_depth:
            self.have_memory = True
            self.memory = Memory(args.num_classes, args.memory_length, dim, dim)
            # self.coarse_mlp = nn.Sequential(nn.Conv1d(dim, dim, kernel_size=1, bias=False),
            #                                 nn.BatchNorm1d(dim, momentum=args.bn_momentum),
            #                                 args.act(),
            #                                 nn.Conv1d(dim, args.num_classes, kernel_size=1))
            self.coarse_mlp = nn.Sequential(nn.Linear(dim, dim, bias=False),
                                            nn.BatchNorm1d(dim, momentum=args.bn_momentum),
                                            args.act(),
                                            nn.Linear(dim, args.num_classes))
            self.loss_fn = PolyFocalLoss(2.0, 0.25, 1.0)
            nn.init.constant_(self.coarse_mlp[-1].bias, 0)
    ...
    def forward(self, x, xyz, prev_knn, pwd, shape_mask_float, y, epoch_ratio):
        """
        x: B x N x C
        """
        # downsampling
        ...
        if self.have_memory:
            ...
            """
            x = x
            # coarse_pred = self.coarse_mlp(x.transpose(1, 2))   # coarse_pred.shape = (b, num_class, n)
            t_b, t_n, t_c = x.shape
            coarse_pred = self.coarse_mlp(x.view(-1, t_c)).view(t_b, t_n, -1).transpose(1, 2)
            coarse_seg_loss = 0
            if self.training:
                coarse_seg_loss = F.cross_entropy(coarse_pred, y, label_smoothing=0.2, ignore_index=255)
                self.memory.update(x, y, coarse_pred, epoch_ratio)
                # print(self.memory.cur_occupy)
            x, _ = self.memory(x, coarse_pred, shape_mask_float)

        # next stage
        if not self.last:
            sub_x, sub_c, sub_coarse_loss = self.sub_stage(x, xyz, knn, pwd, shape_mask_float, y, epoch_ratio)
        else:
            sub_x = None
            sub_c = None
            sub_coarse_loss = None
        ...
        if self.have_memory:
            sub_coarse_loss = sub_coarse_loss + coarse_seg_loss if sub_coarse_loss is not None else coarse_seg_loss

        return sub_x, sub_c, sub_coarse_loss


class DelaPartSeg_Mem(nn.Module):
    r"""
    ...
    def forward(self, xyz, normal, shape, shape_mask, y=None, epoch_ratio=None):
        shape_mask = (1 - shape_mask).to(dtype=torch.bool)
        shape_mask_float = torch.zeros_like(shape_mask, dtype=torch.float32)
        shape_mask_float.masked_fill_(shape_mask, float('-inf'))
        shape_mask_float = shape_mask_float.unsqueeze(dim=1)
        ...
        x, closs, coarse_seg_loss = self.stage(normal, xyz, None, pwd, shape_mask_float, y, epoch_ratio)
        shape = F.one_hot(shape, self.shape_classes).float()
        shape = self.shapeproj(shape).view(B, 1, -1).repeat(1, N, 1).view(B*N, -1)
        x = x + shape
        x = self.head(x)
        if self.training:
            return x, ridx, closs, coarse_seg_loss
        else:
            return x
